...
namespace qgf
{
		...
		void update(float dt)
		{
			...
			if(CollisionCallback)
			{
				...
				for(std::map<long long, GameObject *>::iterator it = worldObjects.begin();it != worldObjects.end();it++)
				{
					...
					{
						int sx = (it->second->m_position.x - it->second->m_sprite->getGlobalBounds().width * 0.5)/dx;
						int sy = (it->second->m_position.y - it->second->m_sprite->getGlobalBounds().height * 0.5)/dy;
						int ex = (it->second->m_position.x + it->second->m_sprite->getGlobalBounds().width * 0.5)/dx;
						int ey = (it->second->m_position.y + it->second->m_sprite->getGlobalBounds().height * 0.5)/dy;
						for(int y=sy;y<=ey;y++)
						{
							for(int x=sx;x<=ex;x++)
							{
	...
	GameObject *ObjectFactory::addObject(const std::string name)
	{
		...
		if(it!=m_objectTemplates.end())
		{
			...
			sf::Texture *im = getImage(it->second.m_imageName);
			if(im)
			{
				gob->m_sprite = new sf::Sprite(*im);
				gob->m_sprite->setTextureRect(sf::IntRect(it->second.m_corner1.x,it->second.m_corner1.y, it->second.m_corner2.x, it->second.m_corner2.y));
				gob->m_sprite->setOrigin(it->second.m_centre.x, it->second.m_centre.y);
			}
			else
			{
				gob->m_sprite = 0;
	...
	sf::Texture *ObjectFactory::getImage(const std::string &filename)
	{
		std::map<std::string, sf::Texture *>::iterator it= m_images.find(filename);
		if(it==m_images.end())
		{
