...
//Edited rect stuff for SFML2 rects.
//Still takes seconds for time
...
namespace qgf
{
	...
	Anim &Anim::update(float deltaT)
	{
		if(m_playing && m_current != m_sequences.end())
		{
			...
			setFrame(((m_current->second.m_end - m_current->second.m_start + 1) / m_current->second.m_length) * m_time + m_current->second.m_start);
		}
		return *this;
	}
